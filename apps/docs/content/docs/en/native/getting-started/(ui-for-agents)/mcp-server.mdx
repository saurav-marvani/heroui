---
title: MCP Server
description: Access Kinetic Native documentation directly in your AI assistant
---

The Kinetic MCP Server gives AI assistants direct access to Kinetic Native component documentation, making it easier to build with Kinetic in AI-powered development environments.
...
<DocsImage
  ...
  alt="Kinetic v3 MCP Server"
/>


The MCP server currently supports **heroui-native** and [stdio transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio). Published at `@kinetic/native-mcp` on npm. View the source code on [GitHub](https://github.com/heroui-inc/heroui-mcp).

<Callout>
  As we add more components to Kinetic Native, they'll be available in the MCP server too.
</Callout>
...
<div className="flex items-center gap-3 mb-4">
  <a
    href="https://link.kinetic-ui.com/native-mcp-cursor-install"
...
```bash
claude mcp add heroui-native -- npx -y @kinetic/native-mcp@latest
```
...
After adding the configuration, restart Claude Code and run `/mcp` to see the Kinetic MCP server in the list. If you see **Connected**, you're ready to use it.

See the [Claude Code MCP documentation](https://docs.claude.com/en/docs/claude-code/mcp) for more details.

**Windsurf:** Add the Kinetic server to your project's `.windsurf/mcp.json` configuration file:
...
**Zed:** Add the Kinetic server to your `settings.json` configuration file. Open settings via Command Palette (`zed: open settings`) or use `Cmd-,` (Mac) / `Ctrl-,` (Linux):
...
**VS Code:** To configure MCP in VS Code with GitHub Copilot, add the Kinetic server to your project's `.vscode/mcp.json` configuration file:
...
**Codex:** Add the Kinetic server to your `~/.codex/config.toml` (or a project-scoped `.codex/config.toml`):
...
**OpenCode:** Add the Kinetic server to your project's `opencode.json` configuration file:
...
```json title="opencode.json"
{
  ...
  "mcp": {
    "heroui-native": {
      "type": "local",
      "command": ["npx", "-y", "@kinetic/native-mcp@latest"]
    }
  }
}
```
...
- "Help me install Kinetic Native in my Expo app"
- "Show me all Kinetic Native components"
...
The MCP server can help you upgrade to the latest Kinetic Native version:

```bash
"Hey Cursor, update Kinetic Native to the latest version"
```
...
| Tool | Description |
|------|-------------|
| `list_components` | List all available Kinetic Native components |
...
| `get_docs` | Browse the full Kinetic Native documentation including guides and principles (use path `/docs/native/getting-started/quick-start` for installation instructions) |
...
- [npm Package](https://www.npmjs.com/package/@kinetic/native-mcp)
