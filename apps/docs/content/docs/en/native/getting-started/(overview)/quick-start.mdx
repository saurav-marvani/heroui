---
title: Quick Start
description: Get up and running with Kinetic Native
icon: rocket
---
...
- **Option 2** — Add Kinetic Native to an existing React Native or Expo project.

## Option 1: Create a New Project

The fastest way to start. The CLI scaffolds an Expo project with Kinetic Native, all required peer dependencies, Uniwind / Tailwind CSS, global styles, and the `KineticNativeProvider` already wired up — so you can jump straight to building.
...
<Callout type="info">
  The scaffold includes Expo with TypeScript, Uniwind + Tailwind CSS preconfigured, `global.css` with the required imports, and the app entry already wrapped in `GestureHandlerRootView` and `KineticNativeProvider`.
</Callout>

## Option 2: Add to an Existing Project

If you already have a React Native or Expo app, follow these steps to install and configure Kinetic Native manually.

<CopyPrompt
  prompt={`Set up Kinetic Native in this React Native project.

Use the Kinetic Native MCP server (@kinetic/native-mcp) as the single source of truth for installation steps, peer dependencies, and component APIs. Start by retrieving the latest quick-start / installation guide from the MCP and follow it precisely — do not rely on memory for versions, package names, or setup steps.
...
- Set up Uniwind + Tailwind CSS (follow the Uniwind quickstart) and add the Kinetic Native imports plus the correct @source line to global.css.
- Make sure GestureHandlerRootView wraps the entire app — it must be the outermost wrapper with style { flex: 1 }.
- Add KineticNativeProvider as a direct child of GestureHandlerRootView, wrapping the rest of the app.

When done, summarize the changes you made and tell me how to run the app on iOS / Android.`}
>
  **Prefer to let your AI assistant do it?** Install the [Kinetic Native MCP Server](/docs/native/getting-started/mcp-server) in your editor, then paste the prompt into your AI assistant — it will analyze your project and handle the entire setup for you.
</CopyPrompt>

### 1. Install Kinetic Native
...
Wrap your application with `KineticNativeProvider`. You must wrap it with `GestureHandlerRootView`:
...
> **Tip**: For even more control over your bundle, consider using [`KineticNativeProviderRaw`](/docs/native/getting-started/provider#raw-provider) — a lightweight provider that excludes `ToastProvider` and `PortalHost`.

## What's Next?

- [Kinetic Native Provider](/docs/native/getting-started/provider)
...
<Callout type="warning">
  Kinetic Native is currently not recommended for web use. We are focusing on mobile platforms (iOS and Android) at this time. For web development, please use [Kinetic React](/docs/react/getting-started/quick-start) instead.
</Callout>
