---
title: Theming
description: Customize Kinetic Native's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
Kinetic Native automatically supports dark mode through [Uniwind](https://docs.uniwind.dev/theming/basics). The theme switches between light and dark variants based on system preferences or manual selection:
...
export default function App() {
  ...
  return <KineticNativeProvider>{/* Your app content */}</KineticNativeProvider>;
}
```
...
All Kinetic Native components automatically use these font variables, ensuring consistent typography throughout your app.

## Variables Reference

Kinetic defines three types of variables:
...
By default, Kinetic Native uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast on soft backgrounds. If you prefer more saturated, vibrant soft foreground colors, import the optional `heroui-native/styles/vibrant` stylesheet alongside the base styles:
