---
title: Animation
description: Add smooth animations and transitions to Kinetic Native components
icon: copy-transparent
---

All animations in Kinetic Native are built with [react-native-reanimated](https://docs.swmansion.com/react-native-reanimated/) and gesture control is handled by [react-native-gesture-handler](https://docs.swmansion.com/react-native-gesture-handler/). It's worth familiarizing yourself with these libraries if you want more control over animations.

## The `animation` Prop

Every animated component in Kinetic Native exposes a single `animation` prop that controls all animations for that component. This prop allows you to modify animation values, timing configurations, layout animations, or completely disable animations.
...
You can disable all Kinetic Native animations globally using the `KineticNativeProvider`:
