...
Kinetic Native uses composition patterns to create flexible, customizable components. Change the rendered element, compose components together, and maintain full control over markup.

## Compound Components

Kinetic Native components use a compound component pattern with dot notation—components export sub-components as properties (e.g., `Button.Label`, `Dialog.Trigger`, `Accordion.Item`) that work together to form complete UI elements.
...
The `asChild` prop lets you change what element a component renders. When `asChild` is true, Kinetic Native clones the child element and merges props instead of rendering its default element.
...
Create your own components by composing Kinetic Native primitives:
