---
title: Styling
description: Style Kinetic Native components with Tailwind or StyleSheet API
icon: brush
links:
  themes: true
  tailwind: theme
...
Kinetic Native components provide flexible styling options: Tailwind CSS utilities, StyleSheet API, and render props for dynamic styling.

## Styling Principles

Kinetic Native is built with `className` as the go-to styling solution. You can use Tailwind CSS classes via the `className` prop on all components.
...
**Using className:** All Kinetic Native components accept `className` props:
...
Each Kinetic Native component exports a `classNames` object that contains the same styling functions used internally by the component. This is particularly useful when you want to style your own custom components to match the appearance of Kinetic Native components.
...
The `classNames` functions accept the same variant props as the components themselves, allowing you to maintain visual consistency across your custom components and Kinetic Native components.


## Responsive Design

Kinetic Native supports Tailwind's responsive breakpoint system via [Uniwind](https://docs.uniwind.dev/breakpoints). Use breakpoint prefixes like `sm:`, `md:`, `lg:`, and `xl:` to apply styles conditionally based on screen width.
...
Kinetic Native provides utility functions to assist with styling components.
