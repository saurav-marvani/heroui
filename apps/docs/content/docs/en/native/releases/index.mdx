---
title: All Releases
description: All updates and changes to Kinetic Native, including new features, fixes, and breaking changes.
---
...
🎉 A new CLI for scaffolding a Kinetic Native project in one command. `npx create-heroui-native-app@latest my-app` generates a complete Expo Router app with Kinetic Native, Uniwind, Tailwind CSS, every required peer dependency, and the provider wiring already in place — pick between a single-screen or tabs starter template.
...
🎉 Kinetic Native reaches its first stable release, graduating from beta and release candidate stages. This milestone includes the new LinkButton component, sub-menu conflict resolution, an optional `@gorhom/bottom-sheet` peer dependency, and improved type safety for `useThemeColor`.
...
This release introduces the Alert compound component with five status variants and accessibility primitives, extracts Radio into a standalone component with dual-mode operation, and adds the animated Select.TriggerIndicator subcomponent. It also delivers KineticNativeProviderRaw for bundle optimization, a `disableFullWindowOverlay` prop for iOS debugging, unified `styles` prop support across six components, and theme surface variable refactoring with explicit per-theme definitions.
...
Kinetic Native follows a regular release cycle:
