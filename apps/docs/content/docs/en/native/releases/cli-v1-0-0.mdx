...
title: CLI v1.0.0
description:
  New CLI for scaffolding a preconfigured Kinetic Native + Expo Router project — Expo SDK 56, React Native 0.85, Uniwind, all peer dependencies wired up, two starter templates (single-screen and tabs).
---
...
🎉 A new CLI — [`create-heroui-native-app`](https://www.npmjs.com/package/create-heroui-native-app) — is now the fastest way to start a Kinetic Native project. One command scaffolds a complete Expo Router app with Kinetic Native, Uniwind, Tailwind CSS, every required peer dependency, and the provider wiring already in place. No manual setup of `global.css`, `metro.config.js`, or `app/_layout.tsx`.
...
| --- | --- | --- |
| `--expo` | `expo` | Single-screen Expo Router app with a Kinetic `Button` demo. |
...
- **Kinetic Native** wrapped in `KineticNativeProvider` and `GestureHandlerRootView` inside `app/_layout.tsx`
- **Uniwind** + **Tailwind CSS** wired through `metro.config.js` and `global.css`
- All Kinetic Native **mandatory peer dependencies** pinned to compatible versions: `react-native-reanimated`, `react-native-gesture-handler`, `react-native-worklets`, `react-native-safe-area-context`, `react-native-svg`, `tailwind-variants`, `tailwind-merge`
- `react-native-screens` so Kinetic overlay components (`Dialog`, `Menu`, `Popover`, `Select`, `BottomSheet`, `Toast`) work out of the box
...
<Callout type="info">
  **Already have an app?** The CLI is for new projects. To add Kinetic Native to an existing React Native or Expo app, follow [Option 2 in the Quick Start](/docs/native/getting-started/quick-start#option-2-add-to-an-existing-project).
</Callout>

## Links

- [Quick Start guide](/docs/native/getting-started/quick-start)
- [Kinetic Native MCP Server](/docs/native/getting-started/mcp-server)
