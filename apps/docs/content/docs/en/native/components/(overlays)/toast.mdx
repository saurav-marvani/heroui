...
Configure toast behavior globally using `KineticNativeProvider` config prop. Global configs serve as defaults for all toasts unless overridden locally.
...
Props for configuring toast behavior globally via `KineticNativeProvider` config prop.
...
Hook to access toast functionality. Must be used within a `ToastProvider` (provided by `KineticNativeProvider`).
...
Toast uses FullWindowOverlay on iOS. To enable the React Native element inspector during development, set `disableFullWindowOverlay={true}` on `ToastProvider` (via `config.toast` when using KineticNativeProvider). Tradeoff: toasts will not appear above native modals when disabled.
