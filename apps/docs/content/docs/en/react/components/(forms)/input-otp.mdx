...
Use the `pattern` prop to restrict input to specific characters. Kinetic exports common patterns like `REGEXP_ONLY_CHARS` and `REGEXP_ONLY_DIGITS`.
...
Kinetic re-exports common regex patterns from input-otp for convenience:
