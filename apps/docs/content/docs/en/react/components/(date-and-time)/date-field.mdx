...
```tsx
import { DateField } from '@kinetic/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@kinetic/react';
...
- **DateField.Input** - Input component with segmented editing from `@kinetic/react`
