---
title: Dark Mode
description: Add light, dark, and system theme switching to Kinetic v3
icon: moon
links:
  themes: true
  tailwind: theme
...
Kinetic dark mode is CSS-driven. Components read theme variables from the root element, so you do not need a Kinetic provider. Add the `dark` class or `data-theme="dark"` to `<html>` and Kinetic will apply the dark theme.
...
<Callout type="info">
  Kinetic's built-in light and dark themes respond to both `.light` / `.dark` classes and `data-theme="light"` / `data-theme="dark"` attributes. If you set both manually, keep the values in sync.
</Callout>
...
Use Kinetic's `useTheme` hook when you are building a plain React app, such as Vite or Create React App, and do not need `next-themes`.

The hook is exported from `@kinetic/react`. It stores the selected theme in `localStorage`, resolves `"system"` from the user's OS preference, and applies both the class and `data-theme` attribute to `<html>`.

```tsx
// src/components/theme-switcher.tsx
import {Button, useTheme} from "@kinetic/react";
...
<Callout type="warning">
  Use one theme controller per app. In Next.js, prefer `next-themes` and its `useTheme` hook. In plain React apps, use `useTheme` from `@kinetic/react`.
</Callout>
