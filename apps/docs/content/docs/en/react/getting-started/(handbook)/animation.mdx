---
title: Animation
description: Add smooth animations and transitions to Kinetic v3 components
icon: copy-transparent
---

Kinetic components support multiple animation approaches: built-in CSS transitions, custom CSS animations, and JavaScript libraries like Framer Motion.

## Built-in Animations

Kinetic components use data attributes to expose their state for animation:
...
Kinetic components work seamlessly with Framer Motion for advanced animations.
...
**Respecting motion preferences:** Kinetic automatically respects user motion preferences using Tailwind's `motion-reduce:` utility. All built-in transitions and animations are disabled when users enable "reduce motion" in their system settings.

Kinetic extends Tailwind's `motion-reduce:` variant to support both the native `prefers-reduced-motion` media query and the `data-reduce-motion` attribute.

```css
/* Kinetic pattern - uses Tailwind's motion-reduce: */
.button {
...
Kinetic automatically detects the user's `prefers-reduced-motion: reduce` setting and disables animations accordingly.
