---
title: Theming
description: Customize Kinetic's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
Kinetic uses CSS variables and [BEM](https://getbem.com/) classes for theming. Customize everything from colors to component styles using standard CSS.
...
Kinetic's theming system is built on top of [Tailwind CSS v4](https://tailwindcss.com/docs/theme)'s theme. When you import `@kinetic/styles`, it uses Tailwind's built-in color palettes, maps them to semantic variables, automatically switches between light and dark themes, and uses CSS layers and the `@theme` directive for organization.
...
> **Dark mode**: For a complete setup guide with `next-themes` and Kinetic's `useTheme` hook, see [Dark Mode](/docs/handbook/dark-mode).
...
@import "@kinetic/styles/base/base.css" layer(base);

/* Theme variables */
@import "@kinetic/styles/themes/shared/theme.css" layer(theme);
@import "@kinetic/styles/themes/default" layer(theme);
/* OR specific theme files */
@import "@kinetic/styles/themes/default/index.css" layer(theme);
@import "@kinetic/styles/themes/default/variables.css" layer(theme);

/* Components (all components) */
@import "@kinetic/styles/components" layer(components);
/* OR specific component files */
@import "@kinetic/styles/components/index.css" layer(components);
@import "@kinetic/styles/components/button.css" layer(components);
@import "@kinetic/styles/components/accordion.css" layer(components);

/* Utilities (optional) */
@import "@kinetic/styles/utilities" layer(utilities);

/* Variants (optional) */
@import "@kinetic/styles/variants" layer(utilities);
```

> **Note**: Directory imports (e.g., `@kinetic/styles/components`) automatically resolve to their `index.css` file. Use explicit file paths (e.g., `@kinetic/styles/components/button.css`) to import individual component styles.

**Headless mode:** Build your own styles from scratch:

```css
@import "tailwindcss";
@import "@kinetic/styles/base/base.css";
...
Kinetic defines three types of variables in [`variables.css`](https://github.com/heroui-inc/heroui/blob/v3/packages/styles/themes/default/variables.css):
...
Kinetic applies a shared scrollbar style to component scroll areas (tables, popovers, drawers, and similar). Scrollbars use standard CSS properties (`scrollbar-width`, `scrollbar-color`, `scrollbar-gutter`) — no `::-webkit-scrollbar` overrides.

**Modes** — set `data-scrollbar` on `<html>`, a component root, or a scroll slot:

| Mode | `data-scrollbar` | Behavior |
|------|------------------|----------|
| Kinetic thin | *(unset)* or `thin` | Thin thumb from theme tokens |
...
<!-- Restore Kinetic scrollbars in one subtree -->
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-thin`, `scrollbar-default`, or `scrollbar-none` utilities from `@kinetic/styles` on your own elements. See [Styling](/docs/handbook/styling) for class-based overrides.

> **Note**: Some components hide scrollbars by default (date picker popovers, color picker, secondary tabs). Nested scroll slots (e.g. the calendar year picker inside a date picker) keep Kinetic scrollbars because `scrollbar-none` only affects the element it is applied to, not descendants using `@apply scrollbar`.

## Vibrant Palette

By default, Kinetic uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast. If you prefer more saturated, vibrant soft foreground colors, add the `data-vibrant-palette` attribute to your root element:
