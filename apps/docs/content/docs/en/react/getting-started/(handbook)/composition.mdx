...
Kinetic uses composition patterns to create flexible, customizable components. Change the rendered element, compose components together, and maintain full control over markup.

## Framework-Agnostic Styles

Kinetic's variant functions are available in the `@kinetic/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use Kinetic's design system:

```tsx
// Import directly from @kinetic/styles (framework-agnostic)
import { buttonVariants } from '@kinetic/styles';

// Or import from @kinetic/react (re-exports the same functions)
import { buttonVariants } from '@kinetic/react';
```

Both imports work identically. Use `@kinetic/styles` when building for non-React frameworks or when you want to avoid pulling in React dependencies.

## Polymorphic Styling

Apply Kinetic styles to any element using variant functions or BEM classes. Extend component styles to framework components, native HTML elements, or custom components with full type safety.
...
When using a custom root element instead of Kinetic's Root component, child components cannot access context slots. You can manually pass `className` to child components using variant functions or BEM classes:
...
This approach works because Kinetic's variant functions and BEM classes can be applied to any element, giving you complete flexibility to style framework components, native elements, or custom components with Kinetic's design system.

## Direct Class Application

The simplest way to style links or other elements is to apply Kinetic's [BEM](https://getbem.com/) classes directly. This approach is straightforward and works with any framework or vanilla HTML.
...
This approach works because Kinetic uses [BEM](https://getbem.com/) classes that can be applied to any element. It's perfect when you don't need the component's interactive features (like `onPress` handlers) and just want the visual styling.

## Using Variant Functions

For more control and type safety, use variant functions to apply Kinetic styles to framework-specific components or custom elements. Variant functions are available from both `@kinetic/styles` (framework-agnostic) and `@kinetic/react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@kinetic/styles`. Use them to apply Kinetic's design system to any element while maintaining type safety.

## Compound Components

Kinetic components are built as compound components—they export multiple parts that work together. Use them in three flexible ways:

**Option 1: Compound pattern (recommended)** — Use the main component directly without `.Root` suffix:

```tsx
import { Alert } from '@kinetic/react';
...
```tsx
import { Alert, AlertTitle, AlertDescription } from '@kinetic/react';
...
// Or named export
import { ButtonRoot } from '@kinetic/react';
...
```tsx
import { Link } from '@kinetic/react';
import { linkVariants, buttonVariants } from '@kinetic/styles';
...
Create your own components by composing Kinetic primitives:
...
```tsx
import type { ButtonRootProps } from "@kinetic/react";
import type { VariantProps } from "tailwind-variants";

import { Button } from "@kinetic/react";
import { buttonVariants, tv } from "@kinetic/styles";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@kinetic/react";
...
```tsx
import { Button, Avatar } from "@kinetic/react";
...
Since `@kinetic/styles` has no React dependencies, you can use it directly in any framework:
```vue
<script setup>
import { buttonVariants } from '@kinetic/styles';
