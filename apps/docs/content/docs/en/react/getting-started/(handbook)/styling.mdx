---
title: Styling
description: Style Kinetic components with CSS, Tailwind, or CSS-in-JS
icon: brush
links:
  themes: true
  tailwind: theme
...
Kinetic components provide flexible styling options: Tailwind CSS utilities, CSS with [BEM](https://getbem.com/) classes or data attributes, CSS-in-JS libraries, and render props for dynamic styling.

## Basic Styling

**Using className:** All Kinetic components accept `className` props:
...
Kinetic scroll slots use `@apply scrollbar` in component CSS. For your own overflow containers, use the utilities from `@kinetic/styles`:
...
| Utility | Effect |
|---------|--------|
| `scrollbar` | Kinetic thumb (reads theme `--scrollbar-*` variables) |
| `scrollbar-thin` | Kinetic themed thin scrollbar |
...
Kinetic components expose their state through data attributes, similar to CSS pseudo-classes:
...
Kinetic uses [BEM methodology](https://getbem.com/) for consistent class naming:
...
```tsx
import { Button as HeroButton, type ButtonProps } from '@kinetic/react';
import { buttonVariants, tv, type VariantProps } from '@kinetic/styles';
...
View all component classes in [@kinetic/styles/components](https://github.com/heroui-inc/heroui/tree/main/packages/styles/components).
