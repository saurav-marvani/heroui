---
title: MCP Server
description: Access Kinetic v3 documentation directly in your AI assistant
---

The Kinetic MCP Server gives AI assistants direct access to Kinetic v3 component documentation, making it easier to build with Kinetic in AI-powered development environments.
...
The MCP server currently supports **@kinetic/react v3** only and [stdio transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio). Published at `@kinetic/react-mcp` on npm. View the source code on [GitHub](https://github.com/heroui-inc/heroui-mcp).

<Callout>
  As we add more components to Kinetic v3, they'll be available in the MCP server too.
</Callout>
...
<div className="flex items-center gap-3 mb-4">
  <a
    href="https://link.kinetic-ui.com/mcp-cursor-install"
...
```bash
claude mcp add heroui-react -- npx -y @kinetic/react-mcp@latest
```
...
Add the Kinetic server to your project's `.windsurf/mcp.json` configuration file:
...
Add the Kinetic server to your `settings.json` configuration file. Open settings via Command Palette (`zed: open settings`) or use `Cmd-,` (Mac) / `Ctrl-,` (Linux):
...
To configure MCP in VS Code with GitHub Copilot, add the Kinetic server to your project's `.vscode/mcp.json` configuration file:
...
Add the Kinetic server to your `~/.codex/config.toml` (or a project-scoped `.codex/config.toml`):
...
Add the Kinetic server to your project's `opencode.json` configuration file:
...
```json title="opencode.json"
{
  ...
  "mcp": {
    "heroui-react": {
      "type": "local",
      "command": ["npx", "-y", "@kinetic/react-mcp@latest"]
    }
  }
}
```
...
- "Help me install Kinetic v3 in my Next.js/Vite/Astro app"
- "Show me all Kinetic components"
...
The MCP server can help you upgrade to the latest Kinetic version:

```bash
"Hey Cursor, update Kinetic to the latest version"
```
...
| Tool | Description |
|------|-------------|
| `list_components` | List all available Kinetic v3 components |
...
| `get_docs` | Browse the full Kinetic v3 documentation including guides and principles (use path `/docs/react/getting-started/quick-start` for installation instructions) |
...
- [npm Package](https://www.npmjs.com/package/@kinetic/react-mcp)
