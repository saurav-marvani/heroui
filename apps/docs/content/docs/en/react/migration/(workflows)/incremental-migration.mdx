...
description: Step-by-step guide for migrating Kinetic v2 to v3 incrementally while keeping both versions working side-by-side
---

## Overview

Incremental migration with coexistence allows you to migrate from Kinetic v2 to v3 component-by-component while keeping your project functional throughout the migration process. This approach uses special setup strategies to allow both v2 and v3 components to work side-by-side.
...
- **Provider**: v2 requires `KineticProvider`, v3 doesn't - you may need conditional provider wrapping
...
- **Using `@kinetic/react`**: Use Strategy A (pnpm aliases)
- **Using component packages** (`@kinetic/button`, `@kinetic/card`, etc.): Use Strategy B (component packages)
...
```json
{
  "dependencies": {
    "@kinetic/react": "2.8.6",
    "@kinetic/theme": "2.4.24",
    "@kinetic-v3/react": "npm:@kinetic/react@latest",
    "@kinetic-v3/styles": "npm:@kinetic/styles@latest"
  }
}
```

2. Import v2 components from `@kinetic/react`:

```tsx
import {Button} from "@kinetic/react"; // v2
```

3. Import v3 components from `@kinetic-v3/react`:

```tsx
import {Button} from "@kinetic-v3/react"; // v3
```

### Migration Process

1. **Migrate one component at a time:**
   - Update imports to use `@kinetic-v3/react`
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@kinetic/react`, `@kinetic/theme`)
   - Remove aliases
   - Update all imports to `@kinetic/react` (remove `-v3` suffix)
...
{
  "dependencies": {
    "@kinetic/react": "latest", // v3
    "@kinetic/styles": "latest", // v3
    "@kinetic/button": "2.8.6", // v2
    "@kinetic/card": "2.8.6", // v2
    // ... other v2 component packages as needed
  }
}
```

2. Import v3 components from `@kinetic/react`:

```tsx
import {Button} from "@kinetic/react"; // v3
```

3. Import v2 components from component packages:

```tsx
import {Card} from "@kinetic/card"; // v2
```

### Migration Process

1. **Install v3 packages** (`@kinetic/react`, `@kinetic/styles`)
2. **Install v2 component packages** for components not yet migrated
3. **Migrate one component at a time:**
   - Remove the v2 component package from dependencies
   - Update imports to use `@kinetic/react` (v3)
   - Update component code to v3 API
   - Test the migrated component
   - For v2 components with no v3 counterpart (Code, Image, Navbar, Ripple, Snippet, Spacer, User), leave them in place. They will continue to work during coexistence.
...
<Callout type="warning">
  **Important:** Import order matters. Import `tailwindcss` first, then `@kinetic/styles`.
</Callout>
...
```js
const {heroui} = require("@kinetic/react");
...
2. **Update imports**
   - Strategy A: Change `@kinetic/react` → `@kinetic-v3/react`
   - Strategy B: Change component package → `@kinetic/react`
...
2. **Guide conflict resolution**
   - Ensure CSS import order: `tailwindcss` first, then `@kinetic/styles`
...
1. **Remove v2 dependencies:**
   - Remove `@kinetic/react` and `@kinetic/theme` (Strategy A)
   - Remove all `@kinetic/*` component packages (Strategy B)
...
2. **Update all imports:**
   - Change `@kinetic-v3/react` → `@kinetic/react` (Strategy A)
   - Change component package imports → `@kinetic/react` (Strategy B)

3. **Update CSS:**
   - Remove v2 Tailwind plugin from config
   - Keep only `@import "@kinetic/styles";`
