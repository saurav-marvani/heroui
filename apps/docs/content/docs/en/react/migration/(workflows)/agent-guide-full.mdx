...
description: Full migration guide for AI assistants helping migrate Kinetic v2 to v3
index: false
---

## Overview

This guide is designed for AI assistants (agents) helping users migrate from Kinetic v2 to v3. It implements a **full migration approach** that analyzes the project first and migrates components in manageable batches rather than all at once.

<Callout type="warning">
  **Critical Constraint**: Kinetic v2 and v3 **should not coexist** in the same project. The project will be broken during migration. Always work in a feature branch and migrate all component code before switching dependencies.
</Callout>
...
- Removing KineticProvider
- Component imports and migration
- Hooks migration
- Styling migration
- Testing

**Agent Note**: Dependency updates (React 19, Tailwind v4) can be done before switching Kinetic (won't break project). However, the Kinetic package switch should only happen AFTER all component code is migrated.
...
1. **Identify hook usage**: Search codebase for imports from `@kinetic/react` that include hook names (`useSwitch`, `useInput`, `useCheckbox`, `useRadio`, `useDisclosure`, etc.)
...
<Callout type="info">
  **Removed/In-Progress/Planned Components**: For components marked as ❌ Removed, 🔄 In Progress, or 📋 Planned, replace them with standard HTML elements during migration. You can migrate back to Kinetic components once they become available in v3.
</Callout>
...
3. **Analyze the project and create a migration plan**
   ...
   - Scan for Kinetic v2 imports, identify all components and usage
...
These steps can be done before switching Kinetic dependencies and won't break the project:

1. **Update React to v19** (if not already)
   - This can be done before switching Kinetic
...
1. **Update dependencies**
   - Remove `@kinetic/react` and `@kinetic/theme` (v2)
   - Install `@kinetic/react` and `@kinetic/styles` (v3)
   - Remove `framer-motion` if present
   - Update CSS imports (add `@import "@kinetic/styles";`)
   - Remove KineticProvider from app root
