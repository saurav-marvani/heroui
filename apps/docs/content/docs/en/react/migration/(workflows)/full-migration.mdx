...
description: Step-by-step guide for migrating Kinetic v2 to v3 using full migration approach
---

## Overview

Full migration is a structured approach to migrating from Kinetic v2 to v3. This method involves migrating all component code first, then switching dependencies, ensuring a clean transition.
...
**Note**: These dependency updates can be done before switching Kinetic (won't break project). However, the Kinetic package switch should only happen AFTER all component code is migrated.

#### Update Kinetic Packages
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm uninstall @kinetic/react @kinetic/theme
    npm install @kinetic/styles @kinetic/react
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm remove @kinetic/react @kinetic/theme
    pnpm add @kinetic/styles @kinetic/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn remove @kinetic/react @kinetic/theme
    yarn add @kinetic/styles @kinetic/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun remove @kinetic/react @kinetic/theme
...
module.exports = {
  content: [
    "./src/**/*.{js,ts,jsx,tsx}",
    "./node_modules/@kinetic/theme/dist/**/*.{js,ts,jsx,tsx}",
  ],
  plugins: [heroui()],
  // ... other config
};
...
Remove the `heroui()` plugin from your Tailwind config. If you only used Tailwind for Kinetic and have no other customizations, you can remove `tailwind.config.js` entirely. Otherwise, keep the file but remove the Kinetic plugin configuration.
...
/* globals.css or main.css */
@import "tailwindcss";
@import "@kinetic/styles"; /* [!code highlight]*/
```

<Callout type="warning">
  Import order matters! Always import `tailwindcss` before `@kinetic/styles`.
</Callout>
...
Kinetic v2 provided component hooks (like `useSwitch`, `useInput`, `useCheckbox`, etc.) and utility hooks like `useDisclosure`. Kinetic v3 removes most component hooks in favor of compound components, and replaces `useDisclosure` with `useOverlayState`.
...
import {Button} from "@kinetic/button";
import {Card} from "@kinetic/card";
...
import {Button, type ButtonProps} from "@kinetic/react";

// Compound component types are properly exported
import {Checkbox, type CheckboxProps} from "@kinetic/react";
...
### Dependencies
- Update React to v19+
- Update Kinetic packages to v3 (after component migration)
- Remove Framer Motion
...
- Remove `KineticProvider` wrapper
- Handle Provider props migration (router, locale, animations)
- Update all component imports to `@kinetic/react`
