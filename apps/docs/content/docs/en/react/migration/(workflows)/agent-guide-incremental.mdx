...
description: Incremental coexistence migration guide for AI assistants helping migrate Kinetic v2 to v3
index: false
---

## Overview

This guide is designed for AI assistants (agents) helping users migrate from Kinetic v2 to v3 using **incremental coexistence migration**. This approach allows v2 and v3 components to work side-by-side, enabling component-by-component migration while keeping the project functional.
...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@kinetic-v3/react": "npm:@kinetic/react@latest"` in package.json
   - **Strategy B**: Look for component-specific packages like `@kinetic/button`, `@kinetic/card` alongside `@kinetic/react`

2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@kinetic/react` (v2) and `@kinetic-v3/react` (v3 alias) are installed
   - Strategy B: `@kinetic/react` (v3) and component packages like `@kinetic/button` (v2) are installed
...
2. **Update imports**
   - Change imports from `@kinetic/react` to `@kinetic-v3/react`
   - Example: `import {Button} from "@kinetic/react"` → `import {Button} from "@kinetic-v3/react"`
...
2. **Remove component package**
   - Remove the v2 component package from dependencies (e.g., `@kinetic/button`)
   - Update package.json

3. **Update imports**
   - Change imports from component package to `@kinetic/react` (v3)
   - Example: `import {Card} from "@kinetic/card"` → `import {Card} from "@kinetic/react"`
...
1. **Remove v2 dependencies**
   - Strategy A: Remove `@kinetic/react`, `@kinetic/theme`, and aliases
   - Strategy B: Remove all remaining `@kinetic/*` component packages
...
2. **Update all imports**
   - Strategy A: Change `@kinetic-v3/react` → `@kinetic/react`
   - Strategy B: All imports should already be `@kinetic/react` (v3)
...
- Projects using unified `@kinetic/react` package (Strategy A can work but full migration may be simpler)
