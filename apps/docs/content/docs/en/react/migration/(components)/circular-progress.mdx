...
description: Migration guide for CircularProgress from Kinetic v2 to v3 (now ProgressCircle)
---

<Callout type="info">
  Refer to the [v3 ProgressCircle documentation](/docs/react/components/progress-circle) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { CircularProgress } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { ProgressCircle, Label } from "@kinetic/react";
