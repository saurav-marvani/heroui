---
title: Autocomplete
description: Migration guide for Autocomplete from Kinetic v2 to v3
---

<Callout type="info">
  In v3, the v2 `Autocomplete` can be migrated to either the [v3 Autocomplete](/docs/react/components/autocomplete) or the [v3 ComboBox](/docs/react/components/combo-box), depending on your use case. See the section below on choosing the right component. This guide focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { Autocomplete, AutocompleteItem } from "@kinetic/react";
...
```tsx
import { Autocomplete, Label, SearchField, ListBox, useFilter } from "@kinetic/react";
...
The v3 Autocomplete uses the `useFilter` hook (from React Aria, re-exported by `@kinetic/react`) to provide locale-aware filtering functions:
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Header, Separator } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { FieldError, Form } from "@kinetic/react";
