---
title: Snippet
description: Migration guide for Snippet from Kinetic v2 to v3
---

<Callout type="warning">
  The Snippet component has been **removed** in Kinetic v3. Use native HTML elements with Tailwind CSS classes and implement copy functionality manually using the Clipboard API.
</Callout>
...
**v2:** `<Snippet>` component from `@kinetic/react`  
...
```tsx
import { Snippet } from "@kinetic/react";
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @kinetic/react");
    setCopied(true);
...
2. **Import Change**: Remove `import { Snippet } from "@kinetic/react"`
...
1. **Remove Import**: Remove `Snippet` from `@kinetic/react` imports
