---
title: TimeInput
description: Migration guide for TimeInput to TimeField from Kinetic v2 to v3
---

<Callout type="info">
  Refer to the [v3 TimeField documentation](/docs/react/components/time-field) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { TimeInput } from "@kinetic/react";
...
```tsx
import { TimeField, DateInputGroup, Label } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Description, FieldError, Label } from "@kinetic/react";
