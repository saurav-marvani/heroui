...
description: Migration guide for DateRangePicker from Kinetic v2 to v3
---

<Callout type="info">
  Refer to the [v3 DateRangePicker documentation](/docs/react/components/date-range-picker) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { DateRangePicker } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { DateField, DateRangePicker, Label, Description, FieldError, RangeCalendar } from "@kinetic/react";
