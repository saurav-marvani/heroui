---
title: Toast
description: Migration guide for Toast from Kinetic v2 to v3
---

<Callout type="info">
  Refer to the [v3 Toast documentation](/docs/react/components/toast) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { ToastProvider, useToast } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Toast, ToastContent, ToastTitle } from "@kinetic/react";
