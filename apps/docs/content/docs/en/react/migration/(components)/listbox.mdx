---
title: Listbox
description: Migration guide for Listbox (renamed to ListBox with a capital "B") from Kinetic v2 to v3
---

<Callout type="info">
  Refer to the [v3 ListBox documentation](/docs/react/components/listbox) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { Listbox, ListboxItem, ListboxSection } from "@kinetic/react";
...
```tsx
import { ListBox, Label } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Description, Label } from "@kinetic/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Header, Label, Separator } from "@kinetic/react";
