---
title: ScrollShadow
description: Migration guide for ScrollShadow from Kinetic v2 to v3
---

<Callout type="info">
  Refer to the [v3 ScrollShadow documentation](/docs/react/components/scroll-shadow) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from Kinetic v2.
</Callout>
...
```tsx
import { ScrollShadow } from "@kinetic/scroll-shadow";
...
**v2:** Imported from `@kinetic/scroll-shadow` package  
**v3:** Imported from `@kinetic/react` unified package
...
- Update import from `@kinetic/scroll-shadow` to `@kinetic/react`
- Enhanced API with more configuration options
- Better shadow detection and visibility control
- Same orientation support (vertical / horizontal); import from `@kinetic/react`
