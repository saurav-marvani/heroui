...
Minor release: Kinetic React docs now ship with Chinese pages and localized demos, soft color tokens were expanded for better readability, scroll containers share a single themeable scrollbar system, and several runtime and layout fixes landed for `useTheme`, Toast, Link, Fieldset, overlays, and RTL layouts.
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @kinetic/styles@latest @kinetic/react@latest
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @kinetic/styles@latest @kinetic/react@latest
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @kinetic/styles@latest @kinetic/react@latest
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @kinetic/styles@latest @kinetic/react@latest
    ```
  </Tab>
</Tabs>
...
<DocsImage
  ...
  alt="Chinese React documentation pages in Kinetic"
...
- Kinetic thin: leave `data-scrollbar` unset, or set `data-scrollbar="thin"` to force the themed thin scrollbar inside a subtree.
...
```html
<div data-scrollbar="thin">Uses the Kinetic themed scrollbar</div>
<div data-scrollbar="default">Uses the browser scrollbar</div>
<div data-scrollbar="none">Hides descendant Kinetic scrollbars</div>
```
...
Use the `scrollbar` utility on any scrollable slot that should follow the active `data-scrollbar` mode. It reads `--scrollbar-width`, `--scrollbar-color`, and `--scrollbar-gutter`, so the same slot can render as Kinetic thin, browser default, or hidden depending on the nearest `data-scrollbar` ancestor.
...
To return to normal browser scrollbars, set `data-scrollbar="default"` on the root `<html>`, your app `<main>`, or any nested container. The mode cascades through descendant Kinetic scroll slots and custom elements using the scrollbar utilities. If you set the root to browser defaults but want one area to use Kinetic styling again, add `data-scrollbar="thin"` on that nested element.

```html
<html data-scrollbar="default">
  <main>
    <section data-scrollbar="thin">
      <!-- Kinetic themed scrollbars here -->
    </section>
  </main>
</html>
```
