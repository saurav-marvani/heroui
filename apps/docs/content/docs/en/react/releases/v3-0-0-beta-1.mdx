...
This release introduces a comprehensive redesign of Kinetic v3, merging v2's beauty and animations with v3's simplicity. All components redesigned, 8 new components, and improved design system with better color tokens, shadows, and architecture.
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @kinetic/styles@beta @kinetic/react@beta
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @kinetic/styles@beta @kinetic/react@beta
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @kinetic/styles@beta @kinetic/react@beta
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @kinetic/styles@beta @kinetic/react@beta
    ```
  </Tab>
</Tabs>

<Callout type="info">
  **Using AI assistants?** Simply prompt "Hey Cursor, update Kinetic to the latest version" and your AI assistant will automatically compare versions and apply the necessary changes. Learn more about the [Kinetic MCP Server](/docs/ui-for-agents/mcp-server).
</Callout>
...
We've spent weeks crafting a new design system that merges the soul of Kinetic v2 with the simplicity of v3. Every component has been redesigned with attention to detail, smooth animations, and improved developer experience. The new design system is available in our [Figma Kit V3](https://www.figma.com/community/file/1546526812159103429/heroui-figma-kit-v3).
...
Kinetic now supports flexible component syntax. Use compound patterns with or without `.Root`, or named exports - all three patterns work identically.
...
// 3. Named exports
import { AvatarRoot, AvatarImage, AvatarFallback } from "@kinetic/react"
...
```tsx
import { Avatar, AvatarFallback } from "@kinetic/react"
...
/* Kinetic automatically calculates these using color-mix */
...
```tsx
import type { AvatarRootProps } from "@kinetic/react"
...
Kinetic now automatically calculates shade levels and soft color variants using CSS `color-mix`. You only need to declare the base colors, and Kinetic handles the rest.
...
/* Kinetic automatically calculates these */
...
/* Kinetic automatically calculates these at 15% opacity */
...
Kinetic Pro is being reshaped from the ground up on top of the new design system. The new Pro version will feature:
- New components built on top of Kinetic v3
...
The reception on the native side has been phenomenal. Thank you for supporting us as we build Kinetic v3! Your feedback helps us improve every day.

See what the community is saying: [Kinetic Native Reception](https://x.com/hero_ui/status/1985721976220966926)
...
- [Kinetic Native](https://link.kinetic-ui.com/native)
