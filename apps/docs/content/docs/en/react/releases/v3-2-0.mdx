...
<Callout type="info">
  **Using AI assistants?** Prompt "Hey Cursor, update Kinetic to the latest version" and your AI assistant will compare versions and apply the necessary changes. Learn more about the [Kinetic MCP Server](/docs/ui-for-agents/mcp-server).
</Callout>
...
<Callout type="warning">
  **Behavior change:** With the default Kinetic theme, tooltip delays now default to `1500ms` / `500ms` instead of the previous React Aria defaults of `700ms` / `0ms`. Set the CSS variables or props explicitly to preserve the old timing.
</Callout>
