...
**Styles separated from implementation:** `@kinetic/styles` is standalone CSS. `@kinetic/react` handles behavior. Use the styles with React, plain HTML + Tailwind, or any framework. BEM class names make every slot customizable globally. Swap themes to change not just variables, but how components look and feel.

**Headless when you want it:** Remove the `@kinetic/styles` import and you have a headless library. We maintain functionality and accessibility. You focus on your product.
...
```css
@import "tailwindcss";
@import "@kinetic/styles/base" layer(base);
@import "@kinetic/styles/themes/default" layer(theme);
@import "@kinetic/styles/components/button.css" layer(components);
@import "@kinetic/styles/components/card.css" layer(components);
```
...
Some users need animations disabled. Kinetic extends Tailwind's `motion-reduce:` variant to support both the system preference and a data attribute:
...
<html data-reduce-motion="true">
  <!-- All Kinetic animations are disabled -->
</html>
```
...
Framer Motion, Motion One, or any CSS animation library works alongside Kinetic's built-in transitions:
...
// Subpath import
import { Button } from "@kinetic/react/button";
import { Card } from "@kinetic/react/card";
import { Table } from "@kinetic/react/table";
```

## UI for Agents

More developers build by prompting than by reading API docs. Kinetic v3 accounts for that.

### MCP Server

The Kinetic MCP Server connects AI coding assistants (Cursor, Claude Code, VS Code Copilot, Windsurf, Zed) to component docs, props, source code, CSS styles, and theme variables. The AI reads the source of truth directly instead of guessing from training data.
...
Ask your AI assistant "update Kinetic to the latest version" and it will compare versions, review the changelog for breaking changes, and apply the necessary code updates automatically.

### Agent Skills

Installable knowledge packs for Cursor and Claude Code. Component patterns, variant usage, theming instructions, and upgrade guides. Preloaded context so the AI writes correct Kinetic code on the first attempt.

### LLMs.txt

Structured documentation files optimized for AI context windows. Published at `/llms.txt` and `/llms-components.txt`, these give any LLM-powered tool a machine-readable summary of Kinetic's API surface.

MCP server, agent skills, LLMs.txt. Three layers that give AI assistants the same access to Kinetic that human developers get from docs.
...
Kinetic Native is a brand-new library shipping alongside v3 for the web. Different rendering engine, same mental model. Where the platforms diverge, the APIs adapt to feel native on each.
...
<div className="flex justify-center">
  **[📱 Open Demo App in Expo Go](https://link.kinetic-ui.com/native-demo)**
</div>
...
If you know Kinetic on the web, most of that knowledge carries over. Familiar component names, dot notation, and prop patterns wherever possible. Where the platforms diverge (layout primitives, gestures, navigation), the APIs adapt to feel native. The mental model stays the same:
...
// App-wide
<KineticNativeProvider config={{ animation: "disable-all" }}>
  <App />
...
```tsx
import { KineticNativeProvider } from "heroui-native/provider";
...
Kinetic Native ships with its own MCP Server, agent skills, and LLMs.txt. Same tooling structure as the web library:
...
Alongside v3, the pre-sale of [Kinetic Pro](https://heroui.pro) is live. Premium components, templates, and AI tooling for both React and React Native.
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  ...
  <Tab value="pnpm">
    ```bash
    pnpm add @kinetic/styles @kinetic/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @kinetic/styles @kinetic/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @kinetic/styles @kinetic/react
    ```
  </Tab>
</Tabs>
