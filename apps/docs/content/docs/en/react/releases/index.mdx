---
title: All Releases
description: All updates and changes to Kinetic v3, including new features, fixes, and breaking changes.
---
...
Patch release: `@kinetic/react` no longer bundles its own copy of `react-aria`. The `react-aria` subpaths are externalized in the Rollup build, so consumers resolve to the single `react-aria` installed in their project instead of a vendored copy. Also fixes `SwitchGroup` horizontal layout.
...
Patch release: new `Text` compound component ported from Kinetic Pro, docs theme selector, `min()`-capped border-radius tokens across 45+ component CSS files, reworked Table focus rings, and fixes for Checkbox, Autocomplete, Tooltip, and form field padding.
...
<DocsImage
  ...
  alt="Kinetic v3 Beta 4"
  href="/docs/react/releases/v3-0-0-beta-4"
/>

<Callout type="warning">
  **Critical Build Issue Fixed**: This version (beta.4) had a critical build issue that has been fixed in **beta.5**. Please upgrade to `@kinetic/styles@3.0.0-beta.5` and `@kinetic/react@3.0.0-beta.5` to ensure proper TypeScript declaration generation and export resolution.
</Callout>
...
- 🎨 **Showcases page** - Gallery of sites built with Kinetic
...
Kinetic v3 is now stable. Future releases follow a regular cycle:
