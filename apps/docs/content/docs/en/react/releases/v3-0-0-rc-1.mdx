...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @kinetic/styles@rc @kinetic/react@rc
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @kinetic/styles@rc @kinetic/react@rc
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @kinetic/styles@rc @kinetic/react@rc
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @kinetic/styles@rc @kinetic/react@rc
    ```
  </Tab>
</Tabs>
...
Table and ListBox now support virtualization via React Aria's `Virtualizer` for large datasets. `Virtualizer`, `TableLayout`, and `ListLayout` are re-exported from `@kinetic/react`.
...
`@kinetic/react` now supports per-component subpath entrypoints for more explicit imports ([#6301](https://github.com/heroui-inc/heroui/pull/6301)):
