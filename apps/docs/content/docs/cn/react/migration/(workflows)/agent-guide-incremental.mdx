---
title: 代理迁移指南 - 共存增量迁移
description: AI 助手增量共存迁移指南，帮助将 Kinetic v2 迁移到 v3
index: false
---

## 概述

本指南专为人工智能助手（代理）设计，帮助用户使用**增量共存迁移**从 Kinetic v2 迁移到 v3。这种方法允许 v2 和 v3 组件并行工作，从而实现逐个组件迁移，同时保持项目正常运行。
...
1. **确定策略**：检查项目是否使用pnpm别名（策略A）或组件包（策略B）
   - **策略 A**：寻找别名，例如`"@kinetic-v3/react": "npm:@kinetic/react@latest"`在 package.json 中
   - **策略 B**：寻找特定于组件的包，例如`@kinetic/button`, `@kinetic/card`旁边`@kinetic/react`

2. **验证设置**：确保共存设置正确：
   - 策略A：两者兼而有之`@kinetic/react`（v2）和`@kinetic-v3/react`（v3 别名）已安装
   - 策略B：`@kinetic/react`(v3) 和组件包，例如`@kinetic/button`(v2) 已安装
...
2. **更新导入**
   - 将导入从 `@kinetic/react` 改为 `@kinetic-v3/react`
   - 例子：`import {Button} from "@kinetic/react"` → `import {Button} from "@kinetic-v3/react"`
...
2. **移除组件包**
   - 从依赖项中删除 v2 组件包（例如，`@kinetic/button`)
   - 更新package.json

3. **更新导入**
   - 将组件包中的导入更改为`@kinetic/react`(v3)
   - 例子：`import {Card} from "@kinetic/card"` → `import {Card} from "@kinetic/react"`
...
1. **删除 v2 依赖项**
   - 策略A：删除`@kinetic/react`, `@kinetic/theme`和别名
   - 策略B：删除所有剩余的`@kinetic/*`组件包
   - 如果项目仍包含上述已移除 v2 组件（**Code**、**Image**、**Navbar**、**Ripple**、**Snippet**、**Spacer**、**User**），请告知用户：他们可稍后在代理协助下按指南移除或替换。

2. **更新所有导入**
   - 策略A：改变`@kinetic-v3/react` → `@kinetic/react`
   - 策略 B：所有导入应已指向 `@kinetic/react`（v3）
...
- 可以接受临时破坏状态的项目
- 喜欢一次性迁移的团队
- 项目使用统一`@kinetic/react`包（策略 A 可以工作，但完全迁移可能更简单）
