---
title: 增量迁移
description: 将 Kinetic v2 增量迁移到 v3 同时保持两个版本并行工作的分步指南
---

## 概述

通过共存的增量迁移，您可以逐个组件地从 Kinetic v2 迁移到 v3，同时保持项目在整个迁移过程中正常运行。此方法使用特殊的设置策略来允许 v2 和 v3 组件并行工作。
...
- **提供商**：v2 需要`KineticProvider`，v3 没有 - 您可能需要条件提供者包装
...
- **使用`@kinetic/react`**：使用策略 A（pnpm 别名）
- **使用组件包** (`@kinetic/button`, `@kinetic/card`等）：使用策略B（组件包）
...
2. 从以下位置导入 v2 组件`@kinetic/react`:

```tsx
import {Button} from "@kinetic/react"; // v2
```

3. 从以下位置导入 v3 组件`@kinetic-v3/react`:
...
3. **迁移所有组件后，切换到仅 v3：**
   - 删除 v2 依赖项（`@kinetic/react`, `@kinetic/theme`)
   - 删除别名
   - 将所有导入更新为`@kinetic/react`（消除`-v3`后缀）
   - 完成样式迁移
...
1. **安装 v3 软件包** (`@kinetic/react`, `@kinetic/styles`)
2. **为尚未迁移的组件安装 v2 组件包**
3. **一次迁移一个组件：**
   - 从依赖中删除 v2 组件包
   - 更新导入以使用`@kinetic/react`(v3)
   - 将组件代码更新为 v3 API
   - 测试迁移的组件
   - 对于没有 v3 对位的 v2 组件（**Code**、**Image**、**Navbar**、**Ripple**、**Snippet**、**Spacer**、**User**），请将它们保留在原处，它们在共存期间仍会继续工作。
4. **继续，直到所有组件均已迁移**
...
<Callout type="warning">
  **重要提示：** 导入顺序很重要：先导入 `tailwindcss`，再导入 `@kinetic/styles`。
</Callout>
...
2. **更新导入**
   - 策略A：改变`@kinetic/react` → `@kinetic-v3/react`
   - 策略 B：更改组件包 →`@kinetic/react`
...
2. **指导冲突解决**
   - 确保 CSS 导入顺序：`tailwindcss`首先，然后`@kinetic/styles`
...
1. **删除 v2 依赖项：**
   - 消除`@kinetic/react`和`@kinetic/theme`（策略A）
   - 全部删除`@kinetic/*`组件包（策略B）
   - 删除别名（策略 A）
...
2. **更新所有导入：**
   - 改变`@kinetic-v3/react` → `@kinetic/react`（策略A）
   - 更改组件包导入 →`@kinetic/react`（策略B）

3. **更新CSS：**
   - 从配置中删除 v2 Tailwind 插件
   - 仅保留`@import "@kinetic/styles";`
