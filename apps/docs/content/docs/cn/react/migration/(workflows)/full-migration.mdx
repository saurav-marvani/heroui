---
title: 完全迁移
description: 使用完整迁移方法将 Kinetic v2 迁移到 v3 的分步指南
---

## 概述

完全迁移是一种从 Kinetic v2 迁移到 v3 的结构化方法。此方法首先迁移所有组件代码，然后切换依赖关系，确保干净的过渡。
...
**注意**：这些依赖项更新可以在切换 Kinetic 之前完成（不会破坏项目）。但是，Kinetic 包切换只能在所有组件代码迁移之后进行。
...
删除`heroui()`来自 Tailwind 配置的插件。如果您仅将 Tailwind 用于 Kinetic 并且没有其他自定义，则可以删除`tailwind.config.js`完全。否则，保留该文件但删除 Kinetic 插件配置。
...
Kinetic v2 提供了组件挂钩（例如`useSwitch`, `useInput`, `useCheckbox`等）和实用程序挂钩，例如`useDisclosure`。 Kinetic v3 删除了大多数组件挂钩，转而使用复合组件，并替换了`useDisclosure`和`useOverlayState`.
...
### 应用程序代码-组件迁移
- 消除`KineticProvider`包装纸
- 处理提供者道具迁移（路由器、区域设置、动画）
- 将所有组件导入更新为`@kinetic/react`
