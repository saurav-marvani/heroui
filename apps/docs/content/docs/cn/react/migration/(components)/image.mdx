---
title: Image
description: Image 从 Kinetic v2 到 v3 的迁移指南。
---

<Callout type="warning">
  Kinetic v3 **已移除** `Image` 组件。请改用原生 HTML `img` 元素或 Next.js 的 `Image` 组件，并结合 Tailwind CSS 类进行样式与效果处理。
</Callout>
...
2. **调整 import**：移除 `import { Image } from "@kinetic/react"`。
...
1. **移除 import**：从 `@kinetic/react` 的 import 中删除 `Image`。
