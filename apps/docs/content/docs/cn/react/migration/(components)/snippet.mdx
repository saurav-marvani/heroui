---
title: Snippet
description: Snippet 从 Kinetic v2 到 v3 的迁移指南。
---

<Callout type="warning">
  Snippet 组件在 Kinetic v3 中已**移除**。请使用原生 HTML 元素配合 Tailwind CSS，并通过 Clipboard API 自行实现复制能力。
</Callout>
...
**v2：** 来自 `@kinetic/react` 的 `<Snippet>` 组件  
...
2. **导入调整**：移除 `import { Snippet } from "@kinetic/react"`。
...
1. **移除导入**：从 `@kinetic/react` 的导入中删除 `Snippet`。
