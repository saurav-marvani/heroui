---
title: Code
description: Code 从 Kinetic v2 到 v3 的迁移指南。
---

<Callout type="warning">
  Code 组件已在 Kinetic v3 中**移除**。请改为使用原生 HTML `<code>` 元素，并配合 Tailwind CSS class 编写样式。
</Callout>
...
2. **导入调整**：移除 `import { Code } from "@kinetic/react"`。
...
1. **移除导入**：从 `@kinetic/react` 的导入中删除 `Code`。
