---
title: Spacer
description: Spacer 从 Kinetic v2 到 v3 的迁移指南。
---

<Callout type="warning">
  Spacer 组件已在 Kinetic v3 中**移除**。请改为直接使用 Tailwind CSS 的 margin 工具类。
</Callout>
...
**v2：** 来自 `@kinetic/react` 的 `<Spacer>` 组件  
...
2. **导入调整**：移除 `import { Spacer } from "@kinetic/react"`。
...
1. **移除导入**：从 `@kinetic/react` 的导入中删除 `Spacer`。
