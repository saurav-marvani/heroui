---
title: MCP服务器
description: 在 AI 助手中访问 Kinetic v2 到 v3 的迁移指南
---

Kinetic 迁移 MCP 服务器使 AI 助手可以直接访问 Kinetic v2 到 v3 的迁移文档。

迁移 MCP 服务器使用 **Streamable HTTP 传输**（基于 URL）并部署在`https://migration-mcp.kinetic-ui.com`。查看源代码[GitHub](https://github.com/heroui-inc/heroui-mcp/tree/main/apps/migration-mcp).

<Callout>
  迁移 MCP 仅用于**迁移**。完成迁移后，切换到[Kinetic React MCP 服务器](/docs/react/getting-started/mcp-server)用于 v3 组件开发。
</Callout>
...
添加配置后，重启 Claude Code 并运行 `/mcp`，在列表中查看 Kinetic 迁移 MCP。若显示 **已连接**，即可开始使用。
...
- “帮我将 Button 从 Kinetic v2 迁移到 v3”
- “获取我的项目的完整迁移工作流程”
- “复选框迁移有何变化？”
...
| 提示词 | 描述 |
|--------|-------------|
| `analyze-and-plan` | 分析项目以识别所有 Kinetic v2 组件并创建迁移计划。建议作为迁移第一步。可选：`migrationType: "full"`（默认）或 `"incremental"` |
