---
title: 迁移
description: 将 Kinetic v2 应用迁移到 v3 的完整指南
index: true
---

## 面向 AI 助手

以下是 AI 助手获取迁移文档的三种方式。***我们推荐使用 Kinetic Migration MCP 服务器***，并充分利用其 prompt 与工具，不过其它方式也都能为 agent 提供完整的文档。
...
- **依赖项**：将 React 升级到 v19+、Kinetic 包升级到 v3、Tailwind CSS 升级到 v4，并移除 Framer Motion
- **无需 Provider**：v3 不再需要 `KineticProvider`
...
如果不做特殊设置，Kinetic v2 与 v3 不能在同一个项目中共存。你可以选择以下两种迁移方式：
