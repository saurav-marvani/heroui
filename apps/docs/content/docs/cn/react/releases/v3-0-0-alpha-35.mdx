...
**迁移步骤：**
1. 单独导入 `SwitchGroup`：`import { Switch, SwitchGroup } from "@kinetic/react"`
...
- Switch 的迁移完成后，请确认 `SwitchGroup` 已被导入：`import { Switch, SwitchGroup } from "@kinetic/react"`
