---
title: 动画
description: 为 Kinetic v3 组件添加流畅的动画和过渡
icon: copy-transparent
---

Kinetic 组件支持多种动画方法：内置 CSS 过渡、自定义 CSS 动画以及 Framer Motion 等 JavaScript 库。

## 内置动画

Kinetic 组件使用数据属性来公开其动画状态：
...
Kinetic 组件与 Framer Motion 无缝协作，实现高级动画。
...
**尊重动态偏好：** Kinetic 使用 Tailwind 的 motion-reduce 工具自动尊重用户动态效果偏好。当用户在系统设置中启用“减少动态效果”时，所有内置的过渡和动画效果都将被禁用。

Kinetic 扩展了 Tailwind 的 motion-reduce: 变体，以同时支持原生 `prefers-reduced-motion` 媒体查询和 `data-reduce-motion` 属性。
...
Kinetic自动检测用户的`prefers-reduced-motion: reduce`相应地设置并禁用动画。
