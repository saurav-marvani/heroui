---
title: 样式
description: 使用 CSS、Tailwind 或 CSS-in-JS 为 Kinetic 组件设置样式
icon: brush
links:
  themes: true
  tailwind: theme
...
Kinetic 组件提供灵活的样式方案：Tailwind CSS 工具类、配合 [BEM](https://getbem.com/) 类名或数据属性的 CSS、CSS-in-JS 库，以及用于动态样式的渲染属性。

## 基础样式

**使用 className：** 所有 Kinetic 组件都接受 `className` 属性：
...
Kinetic 的滚动插槽在组件 CSS 中使用 `@apply scrollbar`。对于你自己的溢出容器，可使用 `@kinetic/styles` 提供的工具类：
...
| 工具类 | 效果 |
|---------|--------|
| `scrollbar` | Kinetic 滚动条滑块（读取主题的 `--scrollbar-*` 变量） |
| `scrollbar-thin` | Kinetic 主题化纤细滚动条 |
...
Kinetic 组件通过数据属性公开其状态，类似于 CSS 伪类：
...
Kinetic 使用 [BEM 方法论](https://getbem.com/) 来保持类名命名的一致性：
...
所有组件的类名请查看 [@kinetic/styles/components](https://github.com/heroui-inc/heroui/tree/main/packages/styles/components)。
