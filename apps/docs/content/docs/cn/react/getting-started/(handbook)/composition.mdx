...
Kinetic 使用组合模式来创建灵活、可定制的组件。你可以更换渲染的元素、把组件组合在一起，并完全掌控最终的标记结构。

## 与框架无关的样式

Kinetic 的变体函数位于 `@kinetic/styles` 包中，可以独立于 React 使用。这使得 Vue、Svelte 等其他框架也能使用 Kinetic 的设计系统：
...
两种导入方式的工作原理完全相同。在为非 React 框架构建，或希望避免引入 React 依赖时，请使用 `@kinetic/styles`。

## 多态样式

使用变体函数或 BEM 类，将 Kinetic 样式应用到任何元素上。可以把组件样式扩展到框架组件、原生 HTML 元素或自定义组件，并保持完整的类型安全。
...
当使用自定义根元素而非 Kinetic 的 Root 组件时，子组件无法访问 context slot。你可以使用变体函数或 BEM 类，手动将 `className` 传递给子组件：
...
这种方法之所以可行，是因为 Kinetic 的变体函数和 BEM 类可以应用到任何元素上，让你能够灵活地用 Kinetic 的设计系统为框架组件、原生元素或自定义组件设置样式。

## 直接应用类名

为链接或其他元素设置样式最简单的方法，就是直接使用 Kinetic 的 [BEM](https://getbem.com/) 类。这种方法简单直接，适用于任何框架或纯 HTML。
...
这种方法之所以可行，是因为 Kinetic 使用了 [BEM](https://getbem.com/) 类，可以应用到任何元素上。当你不需要组件的交互功能（例如 `onPress` 事件处理器）、只想要视觉样式时，这种方式非常合适。

## 使用变体函数

如需更多控制和类型安全，可以使用变体函数将 Kinetic 样式应用到特定框架的组件或自定义元素上。`@kinetic/styles`（与框架无关）和 `@kinetic/react`（重新导出）都提供了变体函数。
...
**可用的变体函数：** 每个组件都从 `@kinetic/styles` 导出其变体函数（`buttonVariants`、`chipVariants`、`linkVariants`、`spinnerVariants` 等）。使用它们可以在保持类型安全的同时，将 Kinetic 的设计系统应用到任何元素上。

## 复合组件

Kinetic 组件以复合组件的方式构建 —— 它们会导出多个协同工作的子部件。你可以通过三种灵活的方式来使用它们：
...
由于 `@kinetic/styles` 没有 React 依赖，你可以直接在任何框架中使用它：
```vue
<script setup>
import { buttonVariants } from '@kinetic/styles';
