---
title: 主题
description: 使用 CSS 变量和全局样式自定义 Kinetic 的设计系统
icon: palette
links:
  themes: true
  tailwind: theme
...
Kinetic 使用 CSS 变量和 [BEM](https://getbem.com/) 类来实现主题化。你可以使用标准 CSS 自定义从颜色到组件样式的所有内容。
...
Kinetic 的主题系统构建于 [Tailwind CSS v4](https://tailwindcss.com/docs/theme) 的主题之上。当你导入 `@kinetic/styles` 时，它会使用 Tailwind 的内置调色板，将其映射到语义化变量，自动在浅色和深色主题之间切换，并使用 CSS 层和 `@theme` 指令进行组织。
...
> **深色模式**：如需基于 `next-themes` 与 Kinetic `useTheme` 钩子的完整配置指南，请参见 [深色模式](/docs/handbook/dark-mode)。
...
> **注意**：目录导入（例如 `@kinetic/styles/components`）会自动解析为其对应的 `index.css` 文件。使用显式文件路径（例如 `@kinetic/styles/components/button.css`）来导入单个组件的样式。
...
Kinetic 在 [`variables.css`](https://github.com/heroui-inc/heroui/blob/v3/packages/styles/themes/default/variables.css) 中定义了三种类型的变量：
...
Kinetic 为组件中的滚动区域（表格、弹出框、抽屉等）应用统一的滚动条样式。滚动条使用标准 CSS 属性（`scrollbar-width`、`scrollbar-color`、`scrollbar-gutter`），不再依赖 `::-webkit-scrollbar` 覆盖。

**模式** — 在 `<html>`、组件根元素或滚动插槽上设置 `data-scrollbar`：

| 模式 | `data-scrollbar` | 行为 |
|------|------------------|------|
| Kinetic 纤细 | *（未设置）* 或 `thin` | 使用主题令牌呈现的纤细滑块 |
...
**自定义溢出区域** — 在自己的元素上使用 `@kinetic/styles` 提供的 `scrollbar`、`scrollbar-thin`、`scrollbar-default` 或 `scrollbar-none` 工具类。基于类的覆盖方式详见 [样式](/docs/handbook/styling)。

> **注意**：部分组件默认隐藏滚动条（日期选择器弹出框、颜色选择器、次级标签页）。嵌套的滚动插槽（例如日期选择器内部的日历年份选择器）会保留 Kinetic 滚动条，因为 `scrollbar-none` 只作用于其所在的元素本身，不会影响使用 `@apply scrollbar` 的后代元素。
