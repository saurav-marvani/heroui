---
title: MCP 服务器
description: 直接在你的 AI 助手中访问 Kinetic v3 文档
---

Kinetic MCP 服务器让 AI 助手可以直接访问 Kinetic v3 组件文档，从而更轻松地在 AI 驱动的开发环境中使用 Kinetic 进行构建。
...
<DocsImage
  ...
  alt="Kinetic v3 MCP 服务器"
/>


MCP 服务器目前仅支持 **@kinetic/react v3**，并使用 [stdio 传输](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio)。已在 npm 上以 `@kinetic/react-mcp` 发布。源代码请查看 [GitHub](https://github.com/heroui-inc/heroui-mcp)。

<Callout>
  随着我们向 Kinetic v3 添加更多组件，它们也将在 MCP 服务器中可用。
</Callout>
...
添加配置后，重启 Claude Code 并运行 `/mcp`，即可在列表中看到 Kinetic MCP 服务器。如果你看到 **Connected**，就可以开始使用了。

更多详情请参阅 [Claude Code MCP 文档](https://docs.claude.com/en/docs/claude-code/mcp)。

### Windsurf

将 Kinetic 服务器添加到你项目的 `.windsurf/mcp.json` 配置文件：
...
将 Kinetic 服务器添加到你的 `settings.json` 配置文件。通过命令面板打开设置（`zed: open settings`），或使用 `Cmd-,`（Mac）/ `Ctrl-,`（Linux）：
...
要在 VS Code 中通过 GitHub Copilot 配置 MCP，请将 Kinetic 服务器添加到项目的 `.vscode/mcp.json` 配置文件：
...
将 Kinetic 服务器添加到你的 `~/.codex/config.toml`（或项目级的 `.codex/config.toml`）：
...
将 Kinetic 服务器添加到你项目的 `opencode.json` 配置文件：
...
- “帮我在 Next.js / Vite / Astro 应用中安装 Kinetic v3”
- “显示所有 Kinetic 组件”
- “Button 组件有哪些 props？”
- “给我一个使用 Card 组件的示例”
- “获取 Button 组件的源代码”
...
| 工具 | 描述 |
|------|-------------|
| `list_components` | 列出所有可用的 Kinetic v3 组件 |
...
| `get_docs` | 浏览完整的 Kinetic v3 文档，包括指南和设计原则（如需安装说明，请使用路径 `/docs/react/getting-started/quick-start`） |
...
- [npm 包](https://www.npmjs.com/package/@kinetic/react-mcp)
