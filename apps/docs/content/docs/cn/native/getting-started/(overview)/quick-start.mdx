---
title: 快速开始
description: 几分钟内上手 Kinetic Native
icon: rocket
---
...
- **方案 2** —— 将 Kinetic Native 添加到已有的 React Native 或 Expo 项目。

## 方案 1：创建新项目

最快捷的启动方式。CLI 会生成一个 Expo 项目，内置 Kinetic Native、所有必需的 peer 依赖、Uniwind / Tailwind CSS、全局样式以及已配置好的 `KineticNativeProvider`，让你可以直接进入开发。
...
<Callout type="info">
  生成的项目自带 Expo + TypeScript、预配置的 Uniwind + Tailwind CSS、含必要导入的 `global.css`，并已在应用入口包裹 `GestureHandlerRootView` 与 `KineticNativeProvider`。
</Callout>

## 方案 2：添加到已有项目

如果你已经有一个 React Native 或 Expo 应用，请按以下步骤手动安装与配置 Kinetic Native。
...
>
  **更喜欢让 AI 助手代劳？** 在编辑器中安装 [Kinetic Native MCP 服务器](/docs/native/getting-started/mcp-server)，将上方提示词粘贴给你的 AI 助手 —— 它会分析你的项目并完成整套配置。
</CopyPrompt>
...
使用 `KineticNativeProvider` 包裹应用，并务必外层再包一层 `GestureHandlerRootView`：
...
> **提示：** 若需要更强控制，可使用 [`KineticNativeProviderRaw`](/docs/native/getting-started/provider#raw-provider) — 轻量 Provider，不包含 `ToastProvider` 与 `PortalHost`。
...
<Callout type="warning">
  Kinetic Native **目前不建议用于 Web**。我们优先聚焦 iOS 与 Android。Web 开发请使用 [Kinetic React](/docs/react/getting-started/quick-start)。
</Callout>
