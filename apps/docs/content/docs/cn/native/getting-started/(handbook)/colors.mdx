---
title: 颜色
description: Kinetic Native 的调色板与主题系统
icon: bucket-paint
links:
  themes: true
  tailwind: theme
...
Kinetic Native 的颜色体系围绕语义意图构建，而非堆砌视觉色板。系统不会暴露庞大的原始色表，而是定义一小套有意义的色彩角色，覆盖绝大多数界面需求。

系统中的多数颜色会由少量基础值自动派生。这样 Kinetic 能在保持对比度、层级与主题行为一致的同时，让整套体系易于理解与修改。
