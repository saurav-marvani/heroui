---
title: 样式
description: 使用 Tailwind 或 StyleSheet API 为 Kinetic Native 组件编写样式
icon: brush
links:
  themes: true
  tailwind: theme
...
Kinetic Native 提供灵活的样式方案：Tailwind CSS 工具类、StyleSheet API，以及用于动态样式的 render props。

## 样式原则

Kinetic Native 以 `className` 作为主要样式入口，所有组件均可通过 `className` 使用 Tailwind 类。
...
**使用 className：** 所有 Kinetic Native 组件都支持 `className`：
...
每个 Kinetic Native 组件都会导出与内部一致的 `classNames` 工具对象，便于让自定义组件在视觉上与库内组件保持一致。
...
`classNames` 函数的变体参数与对应组件一致，便于在自定义组件与 Kinetic 组件之间保持视觉一致。

## 响应式设计

Kinetic Native 通过 [Uniwind](https://docs.uniwind.dev/breakpoints) 支持 Tailwind 的响应式断点前缀，如 `sm:`、`md:`、`lg:`、`xl:`，按屏幕宽度条件应用样式。
