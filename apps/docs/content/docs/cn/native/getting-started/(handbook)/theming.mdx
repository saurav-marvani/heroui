---
title: 主题
description: 使用 CSS 变量与全局样式定制 Kinetic Native 设计系统
icon: palette
links:
  themes: true
  tailwind: theme
...
Kinetic Native 使用 CSS 变量实现主题化，可用标准 CSS 覆盖从颜色到组件样式的一切。
...
通过 [Uniwind](https://docs.uniwind.dev/theming/basics)，Kinetic Native 自动支持深色模式；可跟随系统，也可手动切换明暗变体：
...
所有 Kinetic Native 组件会自动使用这些字体变量，以保持排版一致。
...
默认情况下，Kinetic Native 使用可读性更好的柔和前景色，即将语义色与前景色按比例混合，以在 soft 背景上获得更佳对比度。如果你偏好饱和度更高、更鲜亮的柔和前景色，可以在引入基础样式之后，额外导入可选的 `heroui-native/styles/vibrant` 样式：
