...
```tsx
import { KineticNativeProvider, PortalHost } from "heroui-native";
...
便于在自定义布局中手动挂载 Portal 宿主，例如需在 BottomSheet、Modal 或其他遮罩内指定渲染位置时。默认 `KineticNativeProvider` 已包含标准场景的 `PortalHost`；现可额外创建具名宿主以支持多宿主架构。
