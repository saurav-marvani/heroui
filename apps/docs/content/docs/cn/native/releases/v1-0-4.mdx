...
`@gorhom/bottom-sheet` 的对等依赖范围由 `^5.2.8` 升级到 `^5.2.9`。如果你的应用使用 Kinetic Native 的 [BottomSheet](/docs/native/components/bottom-sheet)（或 `Menu` / `Popover` / `Select` 配合 `presentation="bottom-sheet"`），请确保安装兼容版本：
