---
title: RC 1
description: Alert 组件、独立 Radio 组件、Select TriggerIndicator、KineticNativeProviderRaw、disableFullWindowOverlay、styles 属性扩展、主题 surface 重构
github:
  pull: 284
---
...
RC 1 是 Kinetic Native 的首个候选发布（Release Candidate），表明库已接近生产可用。本版本引入以无障碍为先、带状态变体的 Alert 复合组件；将 Radio 抽为可双模式运行的独立组件；新增带动画的 Select.TriggerIndicator 子组件。还提供轻量 `KineticNativeProviderRaw` 以优化包体、用于 iOS 调试的 `disableFullWindowOverlay`、六个组件统一的 `styles` 插槽式样式，以及用主题显式变量替代计算型 surface 色的主题重构。另含 BottomSheet 内 InputOTP、Toast 文字裁切与元素检查器兼容等关键修复。
...
<Callout type="info">
  **使用 AI 助手？** 直接提示「Hey Cursor，把 Kinetic Native 升级到最新版本」，助手会自动比对版本并完成必要修改。了解更多请参见 [Kinetic Native MCP 服务器](/docs/native/getting-started/mcp-server)。
</Callout>
...
使用 `KineticNativeProvider` 时，Toast 通过 `config.toast` 传入该属性。

**相关 PR：** [#283](https://github.com/heroui-inc/heroui-native/pull/283)

### KineticNativeProviderRaw

新增轻量提供者变体 `KineticNativeProviderRaw`，不包含 `ToastProvider` 与 `PortalHost`，由使用方完全控制打包依赖。
...
- [Provider](/docs/native/getting-started/provider) — KineticNativeProviderRaw 与提供者层级
