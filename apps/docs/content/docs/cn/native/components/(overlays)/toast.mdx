...
toast.show({
  variant: 'success',
  label: '套餐已升级',
  description: '可继续使用 Kinetic Chat',
...
通过 `KineticNativeProvider` 的 `config` 全局配置 Toast；本地调用可覆盖默认值。
...
通过 `KineticNativeProvider` 的 `config.toast` 进行全局配置时可用的属性。
...
访问 Toast 能力，必须在 `ToastProvider` 内使用（由 `KineticNativeProvider` 提供）。
...
Toast 在 iOS 上使用 `FullWindowOverlay`。开发时若需使用 React Native 元素检查器，可在 `KineticNativeProvider` 的 `config.toast` 中设置 `disableFullWindowOverlay={true}`。代价：Toast 将无法叠在原生系统模态之上。
