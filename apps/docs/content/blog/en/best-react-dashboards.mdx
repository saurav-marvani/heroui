...
|---------------------|------|----------------------------|
| **Kinetic Pro Dashboard** | Template + Pro components | Kinetic-native app shell, sidebar, navbar, KPI, data grid, and chart patterns |
...
| **Kinetic Pro + custom widgets** | Pro components + template | Start from Pro layout/table/chart primitives, then add product-specific widgets |

---

## 1. Kinetic Pro Dashboard Template

[Kinetic Pro](https://kinetic-ui.com/pro) offers pre-built dashboard templates designed specifically for SaaS products, admin panels, and analytics dashboards. Built on Kinetic v3 components with Tailwind CSS v4.
...
- Dark mode support with Kinetic's theming system
...
- **AI tooling.** Use Kinetic's MCP server with Cursor or other AI tools to extend and customize dashboards with AI assistance.

**Pricing:** Premium license through Kinetic Pro; check the current pricing page for details.
...
## 14. Kinetic Pro + Custom Widgets

If you want a Kinetic-native dashboard, start with [Kinetic Pro](https://kinetic-ui.com/pro) rather than rebuilding the frame from base components. Use the Pro dashboard template for the app shell and add product-specific widgets only where your product needs something unique.

**What you get:**

- `AppLayout`, `Sidebar`, and `Navbar` for the dashboard frame
- `KPI`, `DataGrid`, `LineChart`, `NumberValue`, and `TrendChip` for dashboard UI
- Base Kinetic components for local UI around the Pro primitives
...
- **Modern stack.** The current dashboard template is built with Next.js and Kinetic Pro components.
- **AI-assisted development.** Use Kinetic's MCP server and docs so agents can extend the dashboard with current APIs.

**How to start:**

1. Start from the Kinetic Pro dashboard template.
...
**Pricing:** Check Kinetic Pro's current pricing and license before choosing it.

**Best for:** Teams that want a Kinetic-native dashboard without rebuilding dashboard infrastructure from scratch.
...
- **Speed to launch:** Kinetic Pro Dashboard or Ant Design Pro give you the most complete starting points.
- **Maximum flexibility:** Kinetic Pro plus custom widgets or the shadcn Dashboard example let you build around your product's exact needs.
...
## Build Your Dashboard with Kinetic

Kinetic provides the components you need to build modern, accessible dashboards:
...
- [Kinetic Pro](https://kinetic-ui.com/pro) — Pre-built dashboard templates and premium components
