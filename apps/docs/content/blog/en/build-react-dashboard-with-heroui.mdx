---
title: "How to Build a React Dashboard with Kinetic Pro"
description: "Create responsive admin dashboards with Kinetic and Kinetic Pro using AppLayout, Sidebar, Navbar, KPI, DataGrid, and chart components."
date: "2026-05-11"
...
I've watched too many teams spend their first two sprints hand-rolling a sidebar, a responsive navbar, and a metrics card layout — only to rip it out later when the requirements get real. If you have Kinetic Pro, skip that phase entirely.
...
Install Kinetic and Kinetic Pro packages:

```bash
npm install @kinetic/styles @kinetic/react @kinetic-pro/react
```

Kinetic Pro requires access to the Pro package registry. The dashboard template uses these style imports:

```css
@import "@kinetic/styles/css";
@import "@kinetic-pro/react/css";
...
The app frame should come from Kinetic Pro `AppLayout`, not a custom flex layout.
...
import {AppLayout} from "@kinetic-pro/react";
...
Kinetic Pro ships a `Sidebar` compound component and a `Navbar` compound component. The dashboard template uses both.
...
import {AppLayout, Navbar, Sidebar} from "@kinetic-pro/react";
...
import {Sidebar} from "@kinetic-pro/react";
...
Do not rebuild metric cards with `Card` unless you need a one-off design. Use `KPI` from Kinetic Pro.

```tsx
"use client";

import {KPI} from "@kinetic-pro/react";
...
Kinetic Pro chart components keep chart styling aligned with the rest of the design system.

```tsx
"use client";

import {Card} from "@kinetic/react";
import {LineChart, NumberValue, TrendChip} from "@kinetic-pro/react";
...
import type {DataGridColumn, DataGridSortDescriptor} from "@kinetic-pro/react";

import {Avatar, SearchField} from "@kinetic/react";
import {DataGrid} from "@kinetic-pro/react";
...
## Why Kinetic for Dashboards

Dashboards are dense — data tables, form controls, navigation, and overlays on a single screen. Kinetic handles the accessibility and styling for all of those pieces so you can focus on your data layer.
...
- **Pro primitives instead of hand-rolled infrastructure.** AppLayout, Sidebar, Navbar, KPI, DataGrid, and chart components from [Kinetic Pro](https://kinetic-ui.com/pro) save weeks of dashboard scaffold work.
...
- Start from the [Kinetic Pro](https://kinetic-ui.com/pro) dashboard template if you have Pro access.
...
- Use base [Kinetic components](/docs/react/components) for local UI that does not need a Pro primitive.
